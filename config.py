...
config.py
=========
All dataclasses for clean parameter management.
This is the foundational branch — every other module imports from here.
"""
...
class RockProperties:
    ...
    porosity: float = 0.20
    permeability: float = 100.0     # mD (uniform or overridden by array)
    compressibility: float = 1e-6   # 1/psi  (rock compressibility)
    perm_array: Optional[np.ndarray] = None   # heterogeneous perm field
...
class FluidProperties:
    ...
    Bw: float = 1.02           # water formation volume factor [rb/stb]
    Bo: float = 1.15           # oil formation volume factor   [rb/stb]
...
class NumericalParams:
    ...
    max_dSw_per_step: float = 0.05   # maximum saturation change per step
    scheme: str = "upwind"     # "upwind" or "minmod"  (flux limiter)


@dataclass
class WellControls:
    """Injection / production well settings."""
...
class SimulationConfig:
    """Top-level container that aggregates all sub-configs."""
    ...
    capillary: CapillaryPressureParams = field(
        default_factory=CapillaryPressureParams
    )
